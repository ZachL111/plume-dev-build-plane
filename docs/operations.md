...
`plume-dev-build-plane` is centered on developer tools work in Rust. The technical target is: Build a Rust toolkit that studies build behavior through negative fixtures, with human-readable error snapshots and no production deployment claims.
...
The score is deterministic. The current threshold is 163. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 4.
